...
export function generateExportWorkbookBuffer(): Buffer {
  ...
  const scansData = rawScans.length > 0 ? rawScans.map((row) => ({
    ...
    'מספר סידורי (S/N)': row.serial_number || 'ללא S/N',
    'מסח"א': row.masha || '',
  ...
  for (const item of anomaliesReport.unauthorizedTransfers || []) {
    flagsData.push({
      ...
      'חומרה': 'גבוהה',
      'מספר סידורי (S/N)': item.serialNumber || 'ללא S/N',
      'מסח"א': item.masha || '',
  ...
  for (const m of anomaliesReport.missingItems || []) {
    flagsData.push({
      ...
      'חומרה': 'גבוהה',
      'מספר סידורי (S/N)': m.serialNumber || 'ללא S/N',
      'מסח"א': m.masha || '',
  ...
  for (const im of anomaliesReport.internalMoves || []) {
    flagsData.push({
      ...
      'חומרה': 'מידע',
      'מספר סידורי (S/N)': im.serialNumber || 'ללא S/N',
      'מסח"א': im.masha || '',
